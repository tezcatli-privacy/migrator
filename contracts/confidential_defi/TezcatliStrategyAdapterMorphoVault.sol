...
/// @notice Adapter for Morpho Vault earn strategies.
/// Morpho Vault V1 and V2 are ERC-4626 compliant, so the integration surface
/// matches the Yearn-style flow while preserving a distinct strategy identity.
contract TezcatliStrategyAdapterMorphoVault is Ownable, ReentrancyGuard {
    ...
    event DeployedToMorpho(uint256 assets, uint256 shares);
    event RedeemedFromMorpho(uint256 shares, uint256 assets);
    ...
    constructor(
        ...
        if (vault_ == address(0) || settlementAsset_ == address(0) || morphoVault_ == address(0)) {
        ...
        if (ERC4626(morphoVault_).asset() != settlementAsset_) {
            revert InvalidAddress();
        }
    }
    ...
    function deploy(uint256 assets, uint256 minSharesOut) external onlyVault nonReentrant returns (uint256 sharesOut) {
        ...
        IERC20(settlementAsset).forceApprove(morphoVault, assets);

        sharesOut = ERC4626(morphoVault).deposit(assets, address(this));
        if (sharesOut < minSharesOut) revert SlippageExceeded();

        managedShares += sharesOut;
        emit DeployedToMorpho(assets, sharesOut);
    }
    ...
    function redeem(
        ...
        assetsOut = ERC4626(morphoVault).redeem(shares, receiver, address(this));
        if (assetsOut < minAssetsOut) revert SlippageExceeded();

        managedShares -= shares;
        emit RedeemedFromMorpho(shares, assetsOut);
    }

    function totalManagedAssets() external view returns (uint256) {
        if (managedShares == 0) return 0;
        return ERC4626(morphoVault).previewRedeem(managedShares);
    }
